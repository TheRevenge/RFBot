		...
		.then(channel => {
			...
			channel.send(embded)
				.then(message => {
					...
					message.channel.awaitMessages(m => m.author.id === guildMember.id, { max: 1, time: 60000, errors: ['time'] })
								...
								.then(response => { // eslint-disable-line no-unused-vars
									...
									channel.send(embded)
										.then(message2 => {
											...
											message2.awaitReactions(filter2, { max: 1, time: 300000, errors: ['time'] })
												.then(() => {
													// Update the in-game motto
													axios.get('https://www.habbo.fr/api/public/users?name=' + gameUsername)
														.then(response2 => {
															...
															if (response2.data.motto.indexOf(code) > -1) {
																...
																axios.get('https://www.habbo.fr/api/public/users/' + response2.data.uniqueId + '/profile')
																	.then(data => {
																		...
																		// If has no HQ
																		if (guildMember.roles.cache.some(r => r.id === ranks.AG) && !guildMember.roles.cache.some(r => r.id === ranks.GD) || guildMember.roles.cache.some(r => r.id === ranks.AS) || guildMember.roles.cache.some(r => r.id === ranks.DA) || guildMember.roles.cache.some(r => r.id === ranks.ST) || guildMember.roles.cache.some(r => r.id === ranks.J)) {
																			embded.setTitle('Succès !')
																				.setDescription('Ton compte a bien été vérifié !');
																			channel.send(embded)
																				.then(message3 => {
																			...
																			channel.send(embded)
																				.then(message3 => {
																					...
																					message3.channel.awaitMessages(m => m.author.id === guildMember.id, { max: 1, time: 60000, errors: ['time'] }).then(collected2 => {
																						...
																						default:
																							embded.setColor('#FF0106')
																								.setTitle('Oops !')
																								.setDescription('Tu as mal écrit le code de ta fiilière, recommence !');
